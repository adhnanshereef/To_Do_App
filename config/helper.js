...
module.exports = {
    ...
    dropProgram:(text,id)=>{
        return new Promise(async(resolve,reject)=>{
            let program=await db.get().collection(collections.PROGRAM_COLLECTION).findOne({_id:objId(id)})
            let drop=await db.get().collection(collections.DROP_COLLECTION).findOne({programId:objId(id)})
            if(drop){
                let textObj={text:program.program[0].text}
                db.get().collection(collections.DROP_COLLECTION).updateOne({programId:objId(id)},{$push:{program:textObj}})
            }else{
               let programDetails={
                programId:objId(id),
                program:[
              ...
              db.get().collection(collections.DROP_COLLECTION).insertOne(programDetails)
            }
                db.get().collection(collections.PROGRAM_COLLECTION).updateOne({_id:objId(id)},{$pull:{program:{text:text}}}).then(async()=>{
                    let programChecking=await db.get().collection(collections.PROGRAM_COLLECTION).findOne({_id:objId(id)}) 
                    if(programChecking.program.length==0){
    ...
    getDrop:()=>{
        return new Promise(async(resolve,reject)=>{
            let drop = await db.get().collection(collections.DROP_COLLECTION).find().toArray()
            resolve(drop)
        })
    },
    deleteAllDrop:()=>{
        return new Promise((resolve,reject)=>{
            db.get().collection(collections.DROP_COLLECTION).deleteMany().then(()=>{
                resolve()
            })
        })
    },
    deleteDroped:(id)=>{
        return new Promise((resolve,reject)=>{
            db.get().collection(collections.DROP_COLLECTION).deleteOne({_id:objId(id)}).then(()=>{
                resolve()
            })
        })
    }
